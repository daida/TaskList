...
class TaskCoordinator: Coordinator {
    ...
    func start() {
        ...
        // `TaskListViewController` expect a `TaskListViewModelInterface` not a concrete type ViewModel
        // So here it will be realy easy to mock the ViewModel by implementing a MockViewModel conform to `TaskListViewModelInterface`
...
extension TaskCoordinator: TaskListViewControllerDelegate {
    func userDidTapOnTask(task: TaskViewModelInterface) {
        ...
        /// so here it will be realy easy to mock the ViewModel by implementing a MockViewModel conform to `TaskViewModelInterface`
